...
class ForecastingModel(pl.LightningModule):
    # Forecasting model with Transformer-based Encoder/Decoder structure
    # Multi-modal predictions made by conditioning on learned embeddings Q
    # Trained with best-of-n loss
    def __init__(
            self,
            num_modes=8,
            num_map_pts=20000,
    ...
    def create_decoder(self):
        self.Q = nn.Parameter(torch.normal(
            torch.zeros((self.num_modes, self.emb_dim)),
            torch.ones((self.num_modes, self.emb_dim))),
            requires_grad=True)

        self.ego_Q = nn.Parameter(torch.normal(
            torch.zeros((self.num_modes, self.emb_dim)),
            torch.ones((self.num_modes, self.emb_dim))),
            requires_grad=True)
    ...
    def get_decoding(self, agents_emb, agents_to_agents_emb, agents_to_agents_masks, scene_to_agents_emb, scene_to_agents_masks):
        B, A, _ = agents_to_agents_masks.shape

        out_seq = repeat(agents_emb, 'b n d -> b n m d', m=self.num_modes)

        ego_out_seq = out_seq[:, :1] + rearrange(self.ego_Q, 'm d -> 1 1 m d')
        other_out_seq = out_seq[:, 1:] + rearrange(self.Q, 'm d -> 1 1 m d')
        out_seq = torch.cat([ego_out_seq, other_out_seq], dim=1)
    ...
    def get_output(self, out_seq):
        ...
        outputs, logits = self.output_model(rearrange(out_seq, 'b a m d -> (b a) m 1 d'))
        outputs = rearrange(outputs, '(b a) m 1 t d -> b m t a d', b=B, a=A)
        logits = rearrange(logits, '(b a) m 1 1 -> b m a', b=B, a=A)
    ...
    def forward(self, obs):
        ...
        out_seq = self.get_decoding(agents_emb, agents_to_agents_emb, agents_masks, scene_to_agents_emb, scene_to_agents_masks)
        outputs, logits = self.get_output(out_seq)
        pred_next_wps = self.get_pred_next_wps(outputs)
        return pred_next_wps, logits, features

    def _get_target_labels(self, mse_error, regression_loss, masked_time_wps_mse_error):
        target_labels = mse_error.argmin(dim=1)
        return target_labels

    def _compute_modes_loss(self, logits, regression_loss, target_labels):
    ...
    def _compute_loss(self, pred_wps, logits, gt_wps, wps_mask):
        ...
        shaped_pred_wps = rearrange(pred_wps, 'b m t a d -> (b m) t a d')
        shaped_gt_wps = repeat(gt_wps, 'b t a d -> (b m) t a d', m=self.num_modes)
        shaped_wps_regression_loss, shaped_wps_mse_errors = self._compute_regression_loss(shaped_pred_wps, shaped_gt_wps)

        time_wps_regression_loss = rearrange(shaped_wps_regression_loss, '(b m) t a -> (b a) m t', b=B, m=self.num_modes)
        time_wps_mse_errors = rearrange(shaped_wps_mse_errors, '(b m) t a d -> (b a) m t d', b=B, m=self.num_modes)

        w_mask = rearrange(wps_mask, 'b t a -> (b a) 1 t 1')
        time_count = torch.clamp(torch.sum(w_mask, dim=2), min=1.)

        wps_regression_loss = torch.sum(time_wps_regression_loss * w_mask.squeeze(dim=-1), dim=2) / time_count.squeeze(dim=-1)
        all_wps_mse_errors = torch.sum(time_wps_mse_errors * w_mask, dim=2) / time_count

        logits = rearrange(logits, 'b m a -> (b a) m')

        all_wps_mse_error = all_wps_mse_errors.sum(dim=-1)

        masked_time_wps_mse_error = torch.sum(time_wps_mse_errors * w_mask, dim=-1)

        target_labels = self._get_target_labels(all_wps_mse_error, wps_regression_loss, masked_time_wps_mse_error)
        unmasked_wps_loss, _, unmasked_logit_loss, labels, target_labels = self._compute_modes_loss(logits, wps_regression_loss, target_labels)

        agents_wps_m = wps_mask.any(dim=1)
        agents_wps_loss = rearrange(unmasked_wps_loss, '(b a) -> b a', b=B, a=A)
        agents_logit_loss = rearrange(unmasked_logit_loss, '(b a) -> b a', b=B, a=A)

        ego_wps_loss = agents_wps_loss[:, :1][agents_wps_m[:, :1]].mean()
        ego_logit_loss = agents_logit_loss[:, :1][agents_wps_m[:, :1]].mean()
        other_wps_loss = agents_wps_loss[:, 1:][agents_wps_m[:, 1:]].mean()
        other_logit_loss = agents_logit_loss[:, 1:][agents_wps_m[:, 1:]].mean()

        wps_loss = 0.5 * (ego_wps_loss + other_wps_loss)
        logit_loss = 0.5 * (ego_logit_loss + other_logit_loss)

        picked_wps_mse_errors = rearrange(all_wps_mse_errors[torch.arange(B * A, device=target_labels.device), target_labels], '(b a) d -> b a d', b=B, a=A)

        picked_time_wps_mse_errors = time_wps_mse_errors[torch.arange(B * A, device=target_labels.device), target_labels]
        ...
        return wps_loss, logit_loss, wps_errors, time_wps_errors, ego_wps_errors, other_wps_errors
    ...
    def run_step(self, batch, prefix):
        ...
        wps_loss, logit_loss, wps_errors, time_wps_errors, ego_wps_errors, other_wps_errors = self._compute_loss(pred_wps, logits, gt_wps, wps_mask)
