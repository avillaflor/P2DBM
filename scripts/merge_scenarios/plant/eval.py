...
def main(cfg):
    ...
    model = model_cls.load_from_checkpoint(cfg.model_checkpoint, **cfg, strict=False)
    ...
    config.populate_config(
        observation_config=EntityObservationConfig(camera=cfg.save_gif, H=1, f=1, max_route_pts=150, max_actors=30, max_token_distance=50.),
    ...
    try:
        for ep in tqdm(range(num_episodes)):
            ...
            if cfg.save_gif:
                if len(ims) > 0:
                    save_dir = os.path.join(cfg.logging_dir, 'plant_viz')
        ...
        eval_save_dir = os.path.join(cfg.logging_dir, 'plant_eval')
